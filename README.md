...
- **Core environment:** Laptop-scale RHEL Docker/Kubernetes/Slurm labs.
- **GPU work:** Use one physical GPU where practical; unsupported multi-node/multi-GPU behavior remains modeled/reference.
...
Where the selected RHEL/Kubernetes environment can safely expose the real NVIDIA GPU, use it. If local GPU passthrough or supported Operator deployment is impractical, keep the cluster CPU-based and model the accelerator control path explicitly.
